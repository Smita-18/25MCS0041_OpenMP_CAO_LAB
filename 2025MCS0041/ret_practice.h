...
void fn_rer_practice1_srl() {
	...
	start = omp_get_wtime();
	printf("Hello coa serial = %d \n", omp_get_thread_num());
	end = omp_get_wtime();
	printf("Execution time for the serial of Hello coa is : %f \n", end - start);
	ETserial = end - start;
}
...
void fn_rer_practice1_prl() {
	...
	start = omp_get_wtime();
#pragma omp parallel
	printf("Hello coa parallel = %d \n", omp_get_thread_num());
	end = omp_get_wtime();
	printf("Execution time for the parallel of Hello coa is : %f \n", end - start);
